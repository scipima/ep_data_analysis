...
To provide some context, we currently have `r length(unique(meps_rcv_mandate$rcv_id))` RCVs in our database, for `r length(unique(meps_rcv_mandate$activity_date))` Plenary dates.
