...
The objective of this short note is to investigate legislative trends during the EP's 9th mandate, and in particular Renew's role in shaping them.
...
### MEPs now ---------------------------------------------------------------------
meps_lastplenary <- data.table::fread(
  here::here("data_out", "meps", "meps_lastplenary.csv"))
```
...
As a rough approximation, RCVs were approximately `r round(n_rcv/n_votes*100, digits=1)`% of the total votes during this mandate.
...
meps_rcv_mandate <- meps_rcv_mandate |> 
  dplyr::left_join(y = data.table::fread(here::here("data_out", "votes", "vote_dict.csv")),
                   by = "result")
meps_rcv_mandate[, result_fct := factor(result_fct,
                                        levels = c("absent", "no vote", "abstain", 
                                                   "against", "for") ) ]
# Tallies
res_tmp <- meps_rcv_mandate[
  !is.na(polgroup_id),
  .N,
...
Out of the legislation that we can trace back to political groups, what's the share of winners? 
...
###--------------------------------------------------------------------------###
# Get House majority
who_won_house <- get_house_majority(data_in = meps_rcv_mandate[!is.na(result)])
...
In a way, these numbers reveal to us the political clout of the different Groups, as they are not about whether a Group agreed with a given piece of legislation, but whether they were able to adopt (or block) one if they agree (or did not agree) with it. 
...
The graph is telling us that, once Renew agreed with the wording of the text (and that occurred about 55.6% of the times), that piece of text was almost always adopted (96.4% of times).


## Groups' divisions
Let us now turn to internal divisions within the EP Political Groups' divisions during the 9th mandate.
```{r}
#| fig-width: 8
#| fig-height: 3
...
cohesion_dt[, .(avg = round( mean(cohesion, na.rm = TRUE), digits = 1)), 
  ...
  ggplot(aes(x=political_group, y = avg)) +
  geom_col(colour = "black", fill = "grey80", linewidth = 0.1) +
  geom_text(aes(label = avg), nudge_y = 3) +
  coord_flip() +
...
So, the Greens were the most united Group during the mandate, followed by the S&D, then EPP and Renew essentially tied, and then all other Groups. 

```{r}
#| eval: false

#' Let's check what difference would have made if ANO, the least-aligned national party had not been a member of Renew. 

meps_rcv_mandate_ano <- meps_rcv_mandate[natparty_id == 5355L, polgroup_id := 5152]
cohesion_dt_ano <- meps_rcv_mandate_ano[result >= -1L, # drop unofficial votes
                                list(
                                  cohesion = cohesion_hn(result_fct),
                                  max_mode = max(tabulate(result_fct)),
                                  tot_votes = sum(tabulate(result_fct)) ), 
                                keyby = list(polgroup_id, rcv_id)] |> 
  join_polit_labs()
cohesion_dt_ano[, .(avg = round( mean(cohesion, na.rm = TRUE), digits = 1)), 
            by = list(political_group)][order(-avg)]
```
