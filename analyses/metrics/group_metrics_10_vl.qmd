...
# Check that we don't miss any vote
if ( all( unique( pl_votes$decision_outcome[pl_votes$rcv_id %in% missing_mjrt]) %in% 
          c("WITHDRAWN", "LAPSED") ) ) {
...
avg_same = vl_renew_mjrt |> 
  dplyr::summarise(avg_same = round(100*mean(is_same, na.rm = TRUE), digits = 2)) |> 
  dplyr::pull(avg_same)
```

The percentage of votes where the Group's majority coincided with the Group's line was: **`r avg_same`%**.

There have been 2 cases where the empirical majority was different from the Group line indicated in the VLs:

* *Délibérations de la commission des pétitions en 2023*; A10-0063/2025 - Gheorghe Falcă - Proposition de résolution (ensemble du texte) (2025-05-07). 
The Group line was to vote `for`, but the Group ended up 
However, in this case, the `Remariks` column stated the following: "*Key votes: If amendment 15, 23 or 24 is not adopted, then abstain for the final vote*".
So, in my opinion, this is not controversial.
