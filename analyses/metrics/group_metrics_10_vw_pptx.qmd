...
cohesion_final_dt <- unique(votes_dt[mandate == 10L, list(rcv_id, is_final)])[
...
dt_plot |> 
  ...
  labs(x="", y="Average Cohesion Rates (%)", fill="",
       caption = "Source: EP. Notes: data is relative to RCVs in Plenary. The height of the bars represent the mean of the distribution of the cohesion rates.") +
  theme(
        axis.text.x = element_text(face="bold", size = 30, angle = 45,
                                   hjust = 1, vjust = 1.3)
  )
...
ggsave(filename = here::here("figures", "cohesion_bycmt_avg_ppt.jpeg"),
       device = "jpeg", dpi = 200, height = 11, width = 23, bg = "white")
```
...
#------------------------------------------------------------------------------#
# Merge RCV with Finals -------------------------------------------------------#
winshare_final_dt <- unique(votes_dt[
  mandate == 10L, 
...
mjrt_renew_groups_cmts_avg |>   
  ...
  facet_wrap(~committee_lab, scales = "free_y", nrow=4) +
  coord_flip() +
  scale_x_reordered() +
  ...
  theme(
    ...
    strip.text = element_text(face = "bold", size = 30, hjust = 0.5),
  )
...
mjrt_renew_parties_avg |>   
  ggplot(aes(x = factor(country_party, levels = plot_rank),
             y = avg) ) +
  geom_col(width=0.6, linewidth=0.1, colour = "black", fill = "#E7298A") +
  geom_text(aes(label = round(avg*100, digits = 1)), 
            hjust=0, vjust = -.7, size = 8, angle=45) + # hjust=0.4) + 
  labs(#title = "Cohesion (%) - All Groups",
    x = "", y = "", fill="") +
  scale_y_continuous(oob=scales::rescale_none, limits = c(0, 1.1),
                     labels = scales::percent) +
  theme(
    axis.text.x = element_text(angle = 60, vjust = 1.1, hjust = 1.1, 
                               face="bold", size = 26)
    )
...
coalition_dt = mjrt_polgroup[
  ...
  dplyr::mutate(
    `Renew + PPE + S&D` = ifelse(test = (Renew == `S&D` & Renew == PPE), 
                                 yes = 1, no = 0),
    `PPE + S&D` = ifelse(test = (PPE == `S&D` & Renew != PPE), yes = 1, no = 0),
    `Renew + PPE` = ifelse(test = (Renew == PPE & Renew != `S&D`), yes = 1, no = 0),
    `Renew + S&D` = ifelse(test = (Renew == `S&D` & Renew != PPE), yes = 1, no = 0),
  ...
  tidyr::pivot_longer(cols = c(`Renew + PPE + S&D`, `PPE + S&D`, `Renew + PPE`, 
                               `Renew + S&D`, `No coalition`, `PPE + ECR`, 
                               `PPE + Far Right`) ) |> 
  dplyr::mutate(is_coalition = ifelse(
    test = name %in% c("Renew + PPE + S&D", "PPE + S&D", "Renew + PPE", 
                       "Renew + S&D", "No coalition"), 
