...
This note includes two tables:

* `Where were we less cohesive` ranks the RCVs where we performed the worst during the last Plenary, and associates to such votes the `Remarks` column coming from our internal Voting Lists.
* `EPP breaches of coalition agreements` lists all cases of EPP's breaches of the coalition agreements, again including the `Remarks` column from our internal Voting Lists.
...
worst_cohesion |> 
  ...
  # sanitizes text content before rendering the table (prevents line breaks)
  dplyr::mutate(
    dplyr::across(where(is.character), ~gsub("[\r\n]+", " ", .x)),
    Rank = dplyr::row_number()
    ) |> 
  dplyr::select(
    Rank,
    Dossier = vote_label_en, 
    `RCV Label` = activity_label_mul, 
    Remarks = remarks,
    `Cohesion rate` = cohesion
    ) |> 
  knitr::kable(align = "c")
```
...
Please note that if one of the Groups did not express a majority for a particular vote, the coalition columns (namely, *(A)* and/or *(B)*) will be labelled as *NA*.
```{r}
#------------------------------------------------------------------------------#
