---
title: "Weber in Gender Issues Analysis"
author:
  - Filipa TORRAO
  - Marco SCIPIONI
date: "`r Sys.Date()`"
...
procedures_df<- data.table::fread(here::here(
...
procedures_title<- procedures_df %>% select(id, process_title_en) # moddify this to the right colname

procedures_docid <- left_join(procedures_docid, procedures_title, by = c("id" = "id"))
...
detailed_info_no_vote<- pl_votes[which(pl_votes$rcv_id %in% rcvs_weber_no_vote), c("rcv_id" ,"activity_date", "number_of_votes_abstention",
                                                                                                    "number_of_votes_against", "number_of_votes_favor", "decision_outcome","doc_id")]

docs_titles <- pl_docs %>% select(doc_id, title_dcterms_en)

detailed_info_no_vote <- left_join(detailed_info_no_vote, docs_titles, by = c("doc_id" = "doc_id"))

colnames(detailed_info_no_vote)<- c("rcv_id","date", "abstentions", "against",
                                               "favour","outcome",
                                               "doc_id", "title")



# Extracting title from procedure title in case it is not already filled in by pl docs
procedures_subset <- procedures_docid %>%
  select(`doc_id`, title_backup = process_title_en)

detailed_info_no_vote$title[which(detailed_info_no_vote$title == "")]<- NA
detailed_info_no_vote <- detailed_info_no_vote %>%
  left_join(procedures_subset, by = "doc_id") %>%
  mutate(title = coalesce(title, title_backup)) %>% 
  select(-title_backup)

#Adding EPP majority, Renew majority, and Weber Party majority
who_won_bygroup <- get_polgroup_majority(
  data_in = meps_rcv_mandate[
    rcv_id %in% rcvs_weber_no_vote
    & result >= -1] # exclude absent
) %>% select(rcv_id, polgroup_id, result)


rcv_epp_result<- who_won_bygroup[which(who_won_bygroup$polgroup_id %in% epp_group_ids), ] %>%
  select(rcv_id,result)
rcv_renew_result<- who_won_bygroup[which(who_won_bygroup$polgroup_id %in% renew_group_ids),] %>%
  select(rcv_id,result)

detailed_info_no_vote <- detailed_info_no_vote %>%
  left_join(rcv_epp_result, by = "rcv_id") 
 
detailed_info_no_vote <- detailed_info_no_vote %>%
  left_join(rcv_renew_result, by = "rcv_id")

detailed_info_no_vote<- detailed_info_no_vote %>% select(
  date, rcv_id, title, result.x, result.y
)

colnames(detailed_info_no_vote)<- c("date","rcv", "title", "epp vote", "renew vote")
detailed_info_no_vote <- detailed_info_no_vote |>
  mutate(rcv = as.character(rcv),
         across(c(`epp vote`, `renew vote`), 
                ~ case_match(.x,
                     1 ~ "+",
                     -1 ~ "-",
                     .default = as.character(.x) # Keep other values (like 0) as text
                )))
#------------------------------------------------------------------------------#
...
detailed_info_vote_against_final<- pl_votes[which(pl_votes$rcv_id %in% rcvs_weber_final_against), c("rcv_id" ,"activity_date", "number_of_votes_abstention",
                                                                                                    "number_of_votes_against", "number_of_votes_favor", "decision_outcome","doc_id")]

docs_titles <- pl_docs %>% select(doc_id, title_dcterms_en)

detailed_info_vote_against_final <- left_join(detailed_info_vote_against_final, docs_titles, by = c("doc_id" = "doc_id"))

colnames(detailed_info_vote_against_final)<- c("rcv_id","date", "abstentions", "against",
                                               "favour","outcome",
                                               "doc_id", "title")
...
detailed_info_vote_against_final$title[which(detailed_info_vote_against_final$title == "")]<- NA
detailed_info_vote_against_final <- detailed_info_vote_against_final %>%
  left_join(procedures_subset, by = "doc_id") %>%
  mutate(title = coalesce(title, title_backup)) %>% 
  select(-title_backup)
...
who_won_bygroup <- get_polgroup_majority(
  data_in = meps_rcv_mandate[
    rcv_id %in% rcvs_weber_final_against
    & result >= -1] # exclude absent
) %>% select(rcv_id, polgroup_id, result)
...
detailed_info_vote_against_final<- detailed_info_vote_against_final %>% select(
  date, rcv_id, title, result.x, result.y
)

colnames(detailed_info_vote_against_final)<- c("date","rcv", "title", "epp vote", "renew vote")
detailed_info_vote_against_final<- detailed_info_vote_against_final |>
  mutate(rcv = as.character(rcv),
         across(c(`epp vote`, `renew vote`), 
                ~ case_match(.x,
