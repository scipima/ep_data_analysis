...
rcvid_lastpl = pl_votes$rcv_id

# Merge with vote labels
pl_votes2 = votes_labels[, list(activity_id, vote_label_en)][
  pl_votes2,
...
# Merge
pl_votes2 = epp_coalitions[
  pl_votes2,
  on = "rcv_id"
]

# Process and print table
epp_breaches = pl_votes2 |> 
  dplyr::filter(
  ...
  dplyr::left_join(
    ...
    by = "rcv_id") |>
  dplyr::select(
    rcv_id, `(A) EPP + ECR + PfE`, `(B) EPP + ECR + ESN + PfE`
  )
...
groups_cohesion = cohesion_final_dt[
  ...
  dplyr::select(
    rank, activity_date, title_dcterms_fr, committee_lab, rcv_id, doc_id, is_final, 
...
groups_cohesion$`(A) EPP + ECR + PfE`[is.na(
                            groups_cohesion$`(A) EPP + ECR + PfE`)]<- "No"
groups_cohesion$`(B) EPP + ECR + ESN + PfE`[is.na(
                            groups_cohesion$`(B) EPP + ECR + ESN + PfE`)]<- "No"
