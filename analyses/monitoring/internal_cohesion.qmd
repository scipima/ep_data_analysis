...
meps_rcv_mandate[
  pers_id %in% 256946L & is.na(natparty_id),
  natparty_id := 6807L
]
...
renew_meps_current = meps_current$pers_id[
  meps_current$polgroup_id == renew_group_ids]
```

The objectives of this short note is to identify **divisions** and **splits** within Renew Europe.
To achieve that, we first calculate the average distance between the Group's majorities and all of its current national parties.
...
# Inner-join ------------------------------------------------------------------#
igroup_iparty <- polgroup_majorities[
  natparty_majorities,
...
```{r}
# Plot Function Average -----------------------------------------------------
group_party_plot <- function(i_polgroup = "Renew") {
  dist_avg_dt |>  
    dplyr::inner_join(
...
# Plot Function Average by Committee ----------------------------------
...
group_party_plot(i_polgroup = "Renew")
```
...
group_party_cmt_plot(i_polgroup = "Renew")
```


### Affinity by MEPs
```{r}
mjrt_renew_meps = polgroup_majorities[
  meps_rcv_mandate[
    pers_id %in% renew_meps_current
    & result >= -1
  ],
  on = "rcv_id"
] |> 
...
mjrt_renew_meps_avg <- mjrt_renew_meps[, list(
  eucl_dist_avg = mean(eucl_dist, na.rm = TRUE) ),
  by = list(country_iso3c, natparty_id, pers_id)
][, `:=`(
  # normalise the euclidean for comparable plotting
  eucl_dist_avg_norm = eucl_dist_avg / 2L
)][order(eucl_dist_avg_norm)] |> 
...
# Inner-join with Committees --------------------------------------------------#
mjrt_renew_meps_cmt <- mjrt_renew_meps[
  rcvid_cmts_10,
  on = "rcv_id",
  nomatch = NULL
]
...
# take the average for plotting
mjrt_renew_meps_cmt_avg <- mjrt_renew_meps_cmt[, list(
  eucl_dist_avg = mean(eucl_dist, na.rm = TRUE) ),
  by = list(country_iso3c, natparty_id, pers_id, committee_lab)
][, `:=`(
  # normalise the euclidean for comparable plotting
  eucl_dist_avg_norm = eucl_dist_avg / 2L
)][order(eucl_dist_avg_norm)]|> 
...
mjrt_renew_meps_avg |> 
    dplyr::mutate(
      national_party = paste(country_iso3c, national_party, sep = " - "),
      mep_name = forcats::fct_reorder(.f = mep_name, .x = eucl_dist_avg_norm) 
      ) |> 
    ggplot(aes(x = eucl_dist_avg_norm, 
               y = mep_name) ) +
    geom_col(fill="grey", colour="black", linewidth=0.1) +
    geom_hline(yintercept = 0, linewidth=0.5) +
  # facet_grid(rows = vars(national_party), space = "free", scales = "free") +
    labs(
      x="", y="Normalised euclidean distance",
      title = "Distance between Group's majority and all of its MEPs",
      subtitle = "The lower the value, the greater the affinity with Group majority.",
      caption = "Source: EP. Notes: data is relative to RCVs in Plenary.") +
    theme_minimal() +
    theme(plot.title.position = "plot",
