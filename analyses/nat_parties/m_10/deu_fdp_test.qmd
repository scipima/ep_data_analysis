...
renew_country_party_long <- national_parties$pref_label_en[
  national_parties$identifier == renew_country_party_id]
renew_country_party_noshow_id <- 6807L
...
For the sake of simplicity, in this analysis we discard MEPs' vote intentions.
```{r}
#| include: false
...
renewparty_deviantmeps <- merge(
  x = who_won_byparty[
    natparty_id %in% renew_country_party_id, # Just target Party
    list(rcv_id, result)],
  y = group_meps_deviations[, list(
    rcv_id, i.result, country_id, polgroup_id, political_group, natparty_id,
    national_party, pers_id, mep_name) ],
  by = "rcv_id", all = FALSE)
...
### Volt alignement with the Green Group
```{r}
