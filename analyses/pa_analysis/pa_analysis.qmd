...
# Read in final votes of the day
final_votes = data.table::fread(file = here::here(
...
#   .SDcols =  cols_outcome]
# # Overwrite `mjrt_labs` if TIE
# rcv_tally_bynatparty_wide[, mjrt_labs := ifelse(
...
#------------------------------------------------------------------------------#
## Subset data based on PARAMS -------------------------------------------------
params_cmts <- automated_vote_analysis_send_list$committee[
  automated_vote_analysis_send_list$working_group == params$working_group
]

if ( params$working_group == "WG-B" ) {
  final_votes <- final_votes |> 
    dplyr::filter(
      committee_lab %in% params_cmts
      | is.na(committee_lab)
    ) |> 
    dplyr::rename(committee_wg = committee_lab_nested) 
} else {
  final_votes <- final_votes |> 
    dplyr::filter(
      committee_lab %in% params_cmts
...
for (i_row in seq_len( nrow(final_votes) ) ) {
  ...
  if ( final_votes$is_multiple_final[i_row] == 1L ){
    cat("This vote refers to ",
        final_votes$activity_label_mul[i_row],
        ".\n\n",
        sep = "")
  }
  ...
  renew_mjrt <- as.character(
    rcv_tally_bygroup_wide$mjrt_labs[
      rcv_tally_bygroup_wide$political_group == "Renew Europe"
      & rcv_tally_bygroup_wide$rcv_id == i_rcvid])
  ...
  if ( "is_pfe_ecr" %in% names(final_votes) ) {
    if ( final_votes$is_pfe_ecr[i_row] == 1L) {
      cat("\nIn this vote, the EPP voted against both S&D and Renew Europe, and together with ECR and PfE.\n")
    } 
  }
  if ( "is_pfe_ecr_esn" %in% names(final_votes) ) {
    if ( final_votes$is_pfe_ecr_esn[i_row] == 1L) {
      cat("\nIn this vote, the EPP voted against both S&D and Renew Europe, and together with ECR, ESN, and PfE.\n")
    }
  }
