...
#------------------------------------------------------------------------------#
## Upload to BigQuery ----------------------------------------------------------
project_dataset <- bigrquery::bq_dataset(project = "ep09-434807",
                                         dataset = "ep_vote_collect")
...
bq_table_update <- function(
    ...
    long_label = "This is the dictionary of EP Political Groups since the EP 9th mandate",
    time_diff = 86400L) {
...
bq_table_update(bq_tbl = bq_national_parties, data_in = national_parties,
                ...
                long_label = "This is the dictionary of national parties since the EP 9th mandate")
# check
...
### MEPs Mandate ---------------------------------------------------------------
# Read in data
meps_mandate <- data.table::fread( here::here("data_out", "meps", "meps_mandate_all.csv") ) |>
...
bq_table_update(bq_tbl = bq_meps_mandate, data_in = meps_mandate,
                short_label = "MEPs during 9th EP Mandate",
                long_label = "Dictionary for all MEPs since the EP 9th mandate")
# check
...
### MEPs Last Plenary ----------------------------------------------------------
# Read in data
meps_lastplenary <- data.table::fread( here::here("data_out", "meps", "meps_lastplenary.csv") )
...
### MEPs Dates -----------------------------------------------------------------
# Read in data
meps_dates_ids <- data.table::fread( here::here("data_out", "meps", "meps_dates_ids_all.csv") ) |>
  dplyr::filter(mandate == 9L)
...
bq_table_update(bq_tbl = bq_meps_dates_ids, data_in = meps_dates_ids,
                short_label = "MEPs' membership over time, 9th mandate",
                long_label = "MEPs' membership in terms of national parties, EP Political Groups, and countries, for each Plenary Session of the 9th mandate")
# check
...
### Votes ----------------------------------------------------------------------
# Read in data
votes_dt <- data.table::fread( here::here("data_out", "votes", "votes_dt_all.csv") ) |>
  dplyr::filter(mandate == 9L)
...
bq_table_update(bq_tbl = bq_votes_dt, data_in = votes_dt,
                short_label = "Votes 9th EP Mandate",
                long_label = "Full list of the voting records in Plenary since the EP 9th mandate")
# check
...
votes_final <- data.table::fread(
  here::here("data_out", "votes", "votes_final_all.csv") ) |>
  dplyr::filter(rcv_id %in% votes_dt$rcv_id) # subset to just 9 mandate
...
bq_table_update(bq_tbl = bq_votes_final, data_in = votes_final,
                short_label = "Final Votes 9th EP Mandate",
                long_label = "Final votes in Plenary since the EP 9th mandate")
# check
...
bq_table_update(bq_tbl = bq_vote_dict, data_in = vote_dict,
                ...
                long_label = "Vote dictionary for both official and calculated votes in Plenary since the EP 9th mandate")
# check
...
plenary_session_docs_rcv <- data.table::fread(
  here::here("data_out", "rcv", "plenary_session_docs_rcv_all.csv")) |>
  dplyr::filter(parliamentary_term == "org/ep-9") # subset to just 9 mandate

rcv_descriptiontext <- plenary_session_docs_rcv |>
  dplyr::select(rcv_id = id, inverse_is_part_of_title_dcterms_fr) |>
...
# Update Table if needed
bq_table_update(bq_tbl = bq_rcv_descriptiontext, data_in = rcv_descriptiontext,
...
bq_table_update(bq_tbl = bq_meps_rcv_mandate, data_in = meps_rcv_mandate,
                short_label = "RCVs 9th EP Mandate",
                long_label = "RCVs since the EP 9th mandate")
# check
...
### Committee Documents --------------------------------------------------------
# Read in data
committee_docs <- data.table::fread( here::here("data_out", "docs_pl", "plenary_docs_committee.csv") )

# Prep
bq_committee_docs <- bigrquery::bq_table(
  project_dataset, "committee_docs", type = "TABLE")

# Update Table if needed
bq_table_update(bq_tbl = bq_committee_docs,
                data_in = committee_docs,
                short_label = "Committee Documents",
                long_label = "Committee Documents wide table")
# check
bigrquery::bq_table_size(bq_committee_docs)


### Committee Lab and DOC id ---------------------------------------------------
# Read in data
docid_committeelab <- data.table::fread( here::here("data_out", "docs_pl", "doc_committee_fromcsv.csv") )

# Prep
bq_docid_committeelab <- bigrquery::bq_table(
  project_dataset, "docid_committeelab", type = "TABLE")

# Update Table if needed
bq_table_update(bq_tbl = bq_docid_committeelab,
                data_in = docid_committeelab,
                short_label = "Committee and DOC id",
                long_label = "Look up table of the links between Committee and DOC id")
# check
bigrquery::bq_table_size(bq_docid_committeelab)


### Plenary Documents ----------------------------------------------------------
# Read in data
plenary_docs <- data.table::fread( here::here(
  "data_out", "docs_pl", "plenary_docs_fromcsv.csv") )
...
bq_table_update(bq_tbl = bq_plenary_docs, data_in = plenary_docs,
                short_label = "All Plenary Documents from CSV",
                long_label = "All Plenary Documents downloaded from CSV")
# check
...
tally_bygroup_byrcv <- data.table::fread( file = here::here(
  ...
  dplyr::filter(rcv_id %in% votes_dt$rcv_id) |> # subset to just 9 mandate
  dplyr::left_join(vote_dict) |>
  tidyr::pivot_wider(id_cols = c(rcv_id, polgroup_id),
              names_from = result_fct, values_from = tally) |>
  dplyr::rename(intention = `NA`)
...
bq_table_update(bq_tbl = bq_tally_bygroup_byrcv, data_in = tally_bygroup_byrcv,
                short_label = "Tallies in RCVs 9th EP Mandate, by Political Groups",
                long_label = "Tallies in RCVs since the EP 9th mandate, by Political Groups, including all types of votes")
# check
...
#   file = here::here("data_out", "aggregates", "tally_bygroup_byparty_byrcv_all.csv")  )
...
#                 short_label = "Tallies in RCVs 9th EP Mandate, by national parties",
#                 long_label = "Tallies in RCVs during the the EP 9th mandate, by national parties, including all types of votes")
# # check
...
majority_bygroup_byrcv <- data.table::fread(
  file = here::here("data_out", "aggregates", "majority_bygroup_byrcv_all.csv") ) |>
  dplyr::filter(rcv_id %in% votes_dt$rcv_id) # subset to just 9 mandate
...
bq_table_update(bq_tbl = bq_majority_bygroup_byrcv,
                ...
                short_label = "Majorities by Political Groups, 9th EP Mandate",
                long_label = "Majorities in RCVs by Political Groups since the EP 9th mandate, including only official votes")
# check
...
whowon_house_polgroup <- data.table::fread(
  file = here::here("data_out", "aggregates", "whowon_house_polgroup_all.csv") ) |>
  dplyr::filter(rcv_id %in% votes_dt$rcv_id) |>  # subset to just 9 mandate|>
  janitor::clean_names()
...
bq_table_update(bq_tbl = bq_whowon_house_polgroup,
                ...
                short_label = "Wins by Political Groups, 9th EP Mandate",
                long_label = "Wins in RCVs by Political Groups since the EP 9th mandate, including only official votes")
# check
