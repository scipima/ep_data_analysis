...
unnest_nested_df <- function(
    data_list = pl_docs_list, # list
    group_col = "id", # string, no more than 1 col
    unnest_col = "workHadParticipation" # string
){
...
unnest_nested_list <- function(
    data_list = pl_docs_list, # list
    group_cols = "id", # string, can be a vector of cols
    unnest_col = "is_derivative_of" # string
) {
  ...
  list_unnest_tmp <- lapply(
    X = data_list, FUN = function(doc) {
      ...
      if ( unnest_col %in% names(doc)
        ...
        setnames(unnest_dt, old = "unnest_col_tmp", new = unnest_col)
